...
2. **Nothing confidential, from anywhere.** No internal tool, service, repo, ticket, channel, or colleague names, and no internal metrics, from any organisation. Work done for someone else lives in its own private repo and is never named here. **Confidential provenance never carries over; public credit does.** If an approach came from a *public* source — an open-source skill, a repo, a post — name it. If it came from anywhere non-public and cannot be described without gesturing at where it came from, leave it out and ask Alex. The enforcement list is deliberately **not** in this repo; it lives in the gitignored `.safety-scan-local.txt`, and those patterns never move into `scan-safety.js`.
   **Clients and prospects are the case this rule is broken in most** *(2026-09-24: a plugin fix found while auditing a prospect's site went out with the prospect's domain in a code comment, its exact sitemap size in the README, and "found running a free report on <domain>" in four commit messages, which then had to be rewritten out of `main`)*. None of these ever names or identifies a client or prospect: **code, comments, READMEs, test fixtures, branch names, or commit messages, merges included.** That covers a company or person name, a domain, and any detail that singles their site out (an exact page count, a distinctive section name). Describe the finding generically: *"on a large sectioned site"*, *"a 28-page site"*. The fix is public; where it came from is not.
...
**Enforcement.** Opt in locally with `git config core.hooksPath .githooks`, which runs the scanner on staged files before every commit (`pre-commit`) and on every commit message, merge commits included (`commit-msg`). `git commit-tree` skips hooks, so scan its message file by hand. The scanner reads `.safety-scan-local.txt` from the working directory or, inside a worktree, from the main checkout; if it finds neither it says so, and client names are not being checked. The `Safety scan` GitHub workflow runs it on every push and pull request, without the local file, so CI never checks client names; the local hooks are the only guard for those. On a fresh machine, recreate `.safety-scan-local.txt` (one literal per line, `#` comments) before committing anything.
